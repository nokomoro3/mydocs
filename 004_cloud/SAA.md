...
- マルチAZ構成は不可。
- 障害時、クラスタの復旧に備えるためには、クロスリージョンスナップショットを設定する必要がある。
  - S3に自動的にスナップショットが保存されるようになる。
- トラフィックを制御したい場合は、拡張VPCルーティングを使用する。
  - 設定しない場合は、その他のAWSサービスなどへのトラフィックが、インターネット経由でルーティングされます。
- 拡張VPCルーティングを有効にすると、通常のVPCと同様以下の設定が可能になります。
  - セキュリティグループ
  - ACL
  - VPCエンドポイント
